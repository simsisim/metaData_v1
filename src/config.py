...
class Config:
    ...
    def find_ticker_file(self, filename):
        """
        Find ticker file with fallback to root directory.
        First checks data/tickers/, then root directory.
...
def find_ticker_file(filename, config):
    """Find ticker file with root directory fallback."""
    from pathlib import Path
    
    # First try in tickers directory
