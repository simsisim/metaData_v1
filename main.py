...
def run_all_basic_calculations(config: Config, user_config: UserConfiguration, timeframes: List[str], clean_file_path: str) -> dict:
    """
    Run basic calculations for all timeframes and all batches.
    New structure: Group by calculation type, not by timeframe.

    Returns:
        dict: Results summary for all timeframes
    """
    print(f"\n" + "="*60)
    print("🔧 BASIC CALCULATIONS - ALL TIMEFRAMES, ALL BATCHES")
    print("="*60)
    ...
    for timeframe in timeframes:
        ...
        (output_base / 'calculations').mkdir(exist_ok=True)

        # Process all batches for basic calculations
        batch_size = getattr(user_config, 'batch_size', 100)
        total_tickers = len(ticker_list)
        total_batches = math.ceil(total_tickers / batch_size)
        ...
        for batch_num in range(total_batches):
            ...
            try:
                basic_calculations(batch_data, output_base / 'calculations', timeframe, user_config, config)
                timeframe_processed += len(batch_data)

            except Exception as e:
                logger.error(f"Error in basic calculations batch {batch_count} for {timeframe}: {e}")
                print(f"❌ Error processing batch {batch_count}: {e}")
                continue
        ...
        if timeframe_processed > 0:
            ...
            # Save matrix immediately after calculations
            print(f"💾 Saving basic calculations matrix for {timeframe}...")
            matrix_result = save_basic_calculations_matrix(config, user_config, timeframe)
            if matrix_result:
                print(f"✅ Matrix saved: {matrix_result['output_file']}")

                # Update centralized data_dates with actual extracted date
                if not hasattr(main, 'data_dates'):
                    main.data_dates = {}
                main.data_dates[timeframe] = {
                    'data_date': matrix_result['data_date'],
                    'formatted_date': matrix_result['formatted_date'],
                    'output_file': matrix_result['output_file']
                }
...
def run_all_stage_analysis(config: Config, user_config: UserConfiguration, timeframes: List[str], clean_file_path: str) -> dict:
    """
    Run stage analysis for all timeframes and all batches.
    Now uses accumulation pattern like basic_calculations.
    """
    print(f"\n" + "="*60)
    print("🎯 STAGE ANALYSIS - ALL TIMEFRAMES, ALL BATCHES")
    print("="*60)

    # Initialize global stage analysis processor
    ...
    for timeframe in timeframes:
        stage_analysis_enabled = getattr(user_config, f'stage_analysis_{timeframe}_enabled', True)
        if not stage_analysis_enabled:
        ...
        # Process all batches for stage analysis
        total_tickers = len(ticker_list)
        total_batches = math.ceil(total_tickers / batch_size)
        timeframe_processed = 0
        ...
        for batch_num in range(total_batches):
            ...
            try:
                ...
                success = processor.process_timeframe(batch_data, timeframe, user_config.ticker_choice)
                if success:
                    timeframe_processed += len(batch_data)

            except Exception as e:
                logger.error(f"Error in stage analysis batch {batch_count} for {timeframe}: {e}")
                print(f"❌ Error processing batch {batch_count}: {e}")
                continue
        ...
        if timeframe_processed > 0:
            ...
            print(f"💾 Saving stage analysis matrix for {timeframe}...")
            matrix_results = processor.save_stage_analysis_matrix(user_config.ticker_choice)
...
def run_all_pvb_screener(config: Config, user_config: UserConfiguration, timeframes: List[str], clean_file_path: str) -> dict:
    """
    Run PVB screener for all timeframes and all batches.
    Now uses accumulation pattern like basic_calculations.
    """
    ...
    print(f"\n" + "="*60)
    print("🔍 PVB SCREENER - ALL TIMEFRAMES, ALL BATCHES")
    print("="*60)

    # Initialize global PVB screener processor
...
def main() -> None:
    ...
    if rs_results.get('status') == 'skipped':
        ...
        print(f"✅ RS analysis completed for {rs_results['universe_size']} tickers")
        print(f"📊 Benchmark: {rs_results['benchmark_ticker']}")
        print(f"🕒 Timeframes processed: {', '.join(rs_results['timeframes_processed'])}")
        ...
        print(f"\n✅ RS files generated containing RS values and returns")
    ...
    # 5. Screeners - All timeframes, all batches (OLD - remaining mixed screeners)
    ...
    print(f"✅ Screeners: {sum(screener_results.values())} total tickers processed")
